...
async function runAudit() {
  ...
  if (missing.length) {
    ...
    console.log(
      `  ❌  ${missing.length} missing key${missing.length !== 1 ? 's' : ''} (used in code, not in any JSON):`
    );
  ...
  if (dynamicCalls.length) {
    console.log(
      `  ⚠   ${dynamicCalls.length} dynamic key call${dynamicCalls.length !== 1 ? 's' : ''} — the unused list may be incomplete:`
    );
  ...
  for (const [lang, list] of Object.entries(untranslated)) {
    console.log(
      `  ⚠   ${lang}: ${list.length} untranslated key${list.length !== 1 ? 's' : ''} (${list.slice(0, 5).join(', ')}${list.length > 5 ? ', …' : ''})`
    );
  }
